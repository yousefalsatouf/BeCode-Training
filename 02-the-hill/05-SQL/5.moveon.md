...
## Installer Adminer
Prends qq minutes pour installer Adminer, un database manager, sur ta machine: https://www.adminer.org/

Pourquoi Adminer ? Parce que ça te permettra de gérer plusieurs types de bases de données : MySQL, MariaDB, PostgreSQL, SQLite, MongoDB... Ce que ne peut pas faire phpMyAdmin qui se limite à MySQL et MariaDB. 

Si tu utilises un container Docker, voici l'image officielle à ajouter : https://hub.docker.com/_/adminer/

## Créer une base de données

Une base de données (ou "db" pour faire court) est essentiellement un nom désignant un ensemble de tables. Du coup, la syntaxe est plutôt simple:
